...
export interface TechPlanet {
  ...
  orbit: 1 | 2 | 3; // 1: Inner, 2: Mid, 3: Outer
  angleOffset: number; // initial degree 0 - 360
  color: string;
...
export const TECH_PLANETS: TechPlanet[] = [
  // Orbit 1: Inner Orbit (Core Frontend & Language)
  {
    id: 'react',
    name: 'React',
    category: 'Frontend',
  ...
  // Orbit 2: Mid Orbit (Backend, APIs & Architecture)
  {
    id: 'nodejs',
    name: 'Node.js',
  ...
  // Orbit 3: Outer Orbit (Databases, Cloud, DevOps & Design)
  {
    id: 'mongodb',
    name: 'MongoDB',
    category: 'Databases',
...
export const TechUniverseOrbit: React.FC<TechUniverseOrbitProps> = ({
  ...
  useEffect(() => {
    const animateOrbits = (time: number) => {
      if (lastTimeRef.current !== null && isOrbiting) {
        const delta = (time - lastTimeRef.current) / 1000;
        setRotationAngle((prev) => ({
          1: (prev[1] + (360 / ORBIT_DURATIONS[1]) * speedMultiplier * delta) % 360,
          2: (prev[2] - (360 / ORBIT_DURATIONS[2]) * speedMultiplier * delta) % 360, // Reverse mid-orbit for dynamic parallax
          3: (prev[3] + (360 / ORBIT_DURATIONS[3]) * speedMultiplier * delta) % 360,
        }));
      }
      lastTimeRef.current = time;
      animFrameRef.current = requestAnimationFrame(animateOrbits);
    };
    ...
    <div className="relative w-full flex flex-col items-center select-none">
      ...
      <div className="relative w-full grid lg:grid-cols-12 gap-5 lg:gap-6 items-center">
        ...
        <div className="lg:col-span-7 flex items-center justify-center relative w-full h-[340px] xs:h-[370px] sm:h-[480px] lg:h-[540px] overflow-hidden rounded-2xl sm:rounded-3xl bg-surface-1/60 border border-primary/20 backdrop-blur-xl shadow-2xl shadow-black/80">
          ...
          {/* Central Full-Stack Star / Reactor Core */}
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            {TECH_PLANETS.map((planet) => {
              ...
              const x = Math.cos(totalAngleRad) * radius;
              const y = Math.sin(totalAngleRad) * radius;
            ...
            {activePlanet ? (
                  ...
                  <div className="flex items-center gap-3.5 mb-3">
                    <div
                      style={{
                        ...
                        boxShadow: `0 0 16px ${activePlanet.color}40`,
                      }}
              ...
              <div className="glass-card p-6 rounded-3xl border border-border/40 bg-surface-1/80 shadow-xl flex flex-col items-center justify-center text-center h-[380px]">
                ...
                <h3 className="text-base font-bold text-foreground mb-1">
                  Explore Subhan's Tech Cosmos
                </h3>
                <p className="text-xs text-muted-foreground max-w-xs leading-relaxed mb-4">
                  Click any orbiting planetary technology in the solar system to explore its architecture, engineering proficiency, and applied portfolio projects.
                </p>
